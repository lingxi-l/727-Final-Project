...
ggplot()+
  ...
  geom_vline(data=elections, aes(xintercept=year), color="blue",size=0.2)+
  geom_text(data=elections, aes(x=year, y=0, label=president),size=2.5, angle=90, vjust=-0.4, hjust=0)+
...
ggplot()+
  geom_line(data=combine,aes(x=year,y=proportion,group=group,color=group))+
  geom_vline(data=raceevents, aes(xintercept=eventyear), color="blue",size=0.2)+
  geom_text(data=raceevents, aes(x=eventyear, y=0, label=event),size=2.5, angle=90, vjust=-0.4, hjust=0)+
  ggtitle("Proportion of Books in Top 200 by Topic")+
  scale_x_continuous(breaks = seq(1960, 2020, by = 4))+
  ylim(0,0.116)+
  theme(axis.text.x = element_text(angle = 45))
